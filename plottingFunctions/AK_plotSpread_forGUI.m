function handles = AK_plotSpread_forGUI(varargin)
...
if nCategories > 1
    ...
    if ~categoryIsLabeled
        ...
        % AMK: dim the greens so I can see them
        plotColors(cellfun(@(x) isequal(x,[0 1 0]),plotColors)) = {[0 .75 0]};
    end
...
if opt.showMM
    ...
    switch opt.xyOri
        case 'normal'
            ...
            if opt.showMM == 5
                mh = plot(ah,opt.xValues,m,'+c','Color','c','MarkerSize',12);
        ...
        case 'flipped'
            ...
            if opt.showMM == 5
                mh = plot(ah,m,opt.xValues,'+c','Color','c','MarkerSize',12);
                mdh = myErrorbar(ah,m,opt.xValues,[sd,NaN(size(sd))]);
            end
    end
end

% AMK: plot fake data on negative x axis so that legend matches categories
for iC = 1:nCategories
    switch opt.xyOri
        case 'normal'
            plot(ah,-1,... % x = -1 to plot off screen
                nanmean(data(:,2)),... % y = mean of data
                'marker',plotMarkers{1,iC},...
                'color',plotColors{1,iC},...
                'lineStyle','none');
        case 'flipped'
            plot(ah,nanmean(data(:,2)),... % x = mean of data
                -1,... % y = -1 to plot off screen
                'marker',plotMarkers{1,iC},...
                'color',plotColors{1,iC},...
                'lineStyle','none');
    end
...
% AMK: set x axis to show only positive x axis (don't show points plotted for legend at x = -1
set(ah,'XLim',[0 nData+1]);
