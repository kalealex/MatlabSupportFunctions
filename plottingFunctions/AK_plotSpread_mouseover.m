function handles = AK_plotSpread_mouseover(varargin)
...
%       mouseover_labels: (added by AMK) assigns mouseover labels to each
%           data point in each distribution (cell array of length
%           distributions, each cell containing labels (cell array of strings)
%           for each data point)
...
elseif ~iscell(varargin{1}) && length(varargin{1}) == 1 && ...
    ...
    data = varargin{2};
    rawData = varargin{2};
    mouseover_labels = varargin{3};
    varargin(1:3) = [];
    newAx = false;    
elseif iscell(varargin{1}) && iscell(varargin{2}) && length(varargin{1}) == length(varargin{2})
    ah = gca;
    data = varargin{1};
    rawData = varargin{1};
    mouseover_labels = varargin{2};
    varargin(1:2) = [];
...
% AMK: also remove from rawData and mouseoverLabels
if ~all(cellfun(@isempty,mouseover_labels))
    for iD = 1:nData;
        if ~isempty(mouseover_labels{iD})
            rawData{iD}(badData(distributionIdx==iD)==1) = [];
            mouseover_labels{iD}(badData(distributionIdx==iD)==1) = [];
        end
    end
end
data(badData) = [];
...
% if there are multiple categories, change structure of rawData and mouseover_labels to match 'idx' below
if nCategories > 1 && ~isempty(mouseover_labels)
    % preallocate for speed
    tempRD = cell(nData,nCategories);
    tempML = cell(nData,nCategories);
    for iD = 1:nData % cycle through distributions
        for iC = 1:nCategories % cycle through categories
            % rearrange rawData and mouseover_labels
            tempRD{iD,iC} = rawData{iD}(categoryIdx(distributionIdx==iD)==iC);
            tempML{iD,iC} = mouseover_labels{iD}(categoryIdx(distributionIdx==iD)==iC);
        end
    end
    % reset rawData and mouseover_labels
    clear rawData mouseover_labels
    mouseover_labels = tempML;
    rawData = tempRD;
    clear tempRD tempML
end
...
if ~isempty(mouseover_labels)
    set(gcf,'windowbuttonmotionfcn',{@mousemove,ph,rawData,mouseover_labels});
end
 

function mousemove(src,ev,L,data,labels)
...
if any(any(obj==L)) %if over any plot... 
    ...
    text(Lrow+xoffset,data{Lindex}(idx)+yoffset,labels{Lindex}{idx},...
